...
function ProcessSelectionScreen({ onLaunch }: { onLaunch: (processes: KycWidgetProcess[]) => void }) {
  const [selectedProcesses, setSelectedProcesses] = useState<KycWidgetProcess[]>([]);

  const toggleProcess = (process: KycWidgetProcess) => {
...
export default function KycWidget() {
  ...
  const handleLaunch = useCallback((processes: KycWidgetProcess[]) => {
    setCreateKycSessionData({ flow: 'verify', processes: processes.length ? processes : undefined });
  ...
  return (
    <div className="kyc-widget-container">
      ...
      {!widgetResult && !createKycSessionData && <ProcessSelectionScreen onLaunch={handleLaunch} />}

      {!widgetResult && createKycSessionData && (
        <>
      ...
      {widgetResult && <ResultScreen onBack={handleBack} widgetResult={widgetResult} />}
    </div>
  );
}
